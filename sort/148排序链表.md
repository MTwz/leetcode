...
class Solution {
    public ListNode sortList(ListNode head) {
		 ...
		 * （4）几个边界条件：每一轮（相同mergeCount）下终止条件：<1>恰好归并完成（例如总长度为7，一个长度为4的子链表+一个长度为3的子链表归并）
		 *                                             <2>找不到第二条链表（break条件）（该条件即使是在第一条子链表满足长度条件的前提下也会break）
		 *                                                （此处secondMergeHead.next!=null与secondMergeHead!=null均是**错误**的）
		 *                                                （原因是：当第一条恰好满足条件的时候，secondMergeHead指向null，但whilr条件不会让secondMergeHead指向null）
		 * 几个注意：
